...
# Read in 4 PAF files into labeled data frames
paf2 <- read.table("comparisons/OR062525_1_vs_OP.paf") 
paf4 <- read.table("comparisons/OR062527_1_vs_OP.paf") # no paf1,3,6 alignments
paf5 <- read.table("comparisons/OR062528_1_vs_OP.paf")
paf7 <- read.table("comparisons/OR062530_1_vs_OP.paf")

paf_list <- list(paf2, paf4, paf5, paf7)
...
reference_length <- paf2$length2[1]                           # Reference genome length
reference_name <- paf2$seq_id2[1]                             # Reference genome name
...
y_positions <- seq(1, by = 2, length.out = 4) #7 for MK, 6 for NC, 
...
ref_genera <- data.frame(
  name = "Genus: Vequintavirus", # Phapecoctavirus Tequatrovirus Vequintavirus
  start = 0,
  end = 0,
  y = 9)
...
# Saving plots
ggsave("plots/vequintavirus.jpg", plot = plotme, device = "jpeg", width = 7, height = 3.5, dpi = 300)
