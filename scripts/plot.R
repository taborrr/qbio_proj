...
paf3 <- read.table("comparisons/OR062526_1_vs_MK.paf")
paf4 <- read.table("comparisons/OR062527_1_vs_MK.paf")
paf4 <- read.table("comparisons/OR062527_1_vs_MK.paf")
paf5 <- read.table("comparisons/OR062528_1_vs_MK.paf")
...
##################################################################
##################################################################
# TESTING FUNCTION SPLIT_INTERVALS ON TINY DATA FRAME

# Example tiny version of align_rects with all loophole error conditions
align_rects_v1 <- data.frame(
  query_names = c("OR1", "OR2", "OR3", "or4"),
  start = c(59900, 136500, 500, 70001),
  end = c(59999, 147010, 20500, 80000),
  y = c(1, 2, 3, 4),
  r_start = c(48900, 56500, 2501, 120601),
  r_end = c(48999, 67010, 22501, 130600)
)
# Create a new data frame to store the processed rows
new_align_rects <- data.frame()

# Process each row in the original data frame
for (i in 1:nrow(align_rects_v1)) {
  print(paste("Processing row", i))
  row <- align_rects_v1[i, ]
  print("Current row:")
  print(row)
